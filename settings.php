...
// Modify query to include search functionality with JOIN
$query = "
    SELECT u.*, b.entry_date 
    FROM users u
    JOIN birth_information b ON u.user_id = b.user_id
...
if (!$result) {
    die("Database query failed: " . mysqli_error($conn)); // Handle the error appropriately
}
?>
...
<section id="settings-section">
    ...
    <table class="settings-table">
        <thead>
            <tr>
                ...
                <th><i class="fas fa-user-check"></i> Entry Date</th>
    ...
    ?>
        <tr>
            ...
            <td>
                <input type="password" id="password-<?php echo $row['user_id']; ?>" class="password-field" value="<?php echo htmlspecialchars($row['password']); ?>">
                <i class="fas fa-eye" onclick="togglePassword(<?php echo $row['user_id']; ?>)" style="cursor: pointer;"></i>
            </td>
            <td><?php echo htmlspecialchars($row['email']); ?></td>
            <td><?php echo htmlspecialchars($row['entry_date']); ?></td>
            <td><a class="edit-button" href="edit.php?id=<?php echo $row['user_id']; ?>">Edit</a></td>
            <td><a class="delete-button" href="delete.php?id=<?php echo $row['user_id']; ?>">Delete</a></td>
        </tr>
    <?php
    }
    ?>
